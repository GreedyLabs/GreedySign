...
// ─── Types ─────────────────────────────────────────────────
type ParticipantRole = 'signer' | 'cc';
...
// ─── Icons ────────────────────────────────────────────────
// CheckIcon 은 `components/ui/Icon` 에서 import. UploadCloudIcon 은 이 페이지
// 에서만 쓰여 지역에 둔다.
...
function ParticipantSetup({ doc }: ParticipantSetupProps) {
  ...
  const [suggestions, setSugg] = useState<UserSuggestion[]>([]);
    ...
    timer.current = setTimeout(async () => {
      try {
        ...
        setSugg(
          data.filter(
            (u) =>
              u.email !== ownerEmail && !participants.find((p) => p.email === u.email),
          ),
        );
      } catch {
        /* ignore */
  ...
  const addParticipant = (email: string, name = '') => {
  ...
  const updateParticipantRole = (email: string, newRole: ParticipantRole) => {
    setParticipants((prev) =>
      prev.map((p) => (p.email === email ? { ...p, role: newRole } : p)),
    );
  };
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      if (!ownerIsSigner) {
        // 소유자를 cc로 변경 (서명 불필요)
        const { data: parts } = await api.get<DocParticipant[]>(
          `/documents/${doc.id}/participants`,
        );
        const ownerPart = parts.find((p) => p.is_owner);
        if (ownerPart) {
          await api.patch(`/documents/${doc.id}/participants/${ownerPart.id}`, {
            role: 'cc',
          });
        }
      }
      ...
      for (const p of participants) {
        try {
          ...
          hasError = true;
          setError((prev) => prev + `\n${p.email}: ${errorMessage(err, '추가 실패')}`);
        }
      }
            ...
            return (
              ...
              >
                ...
                <select
                  value={p.role}
                  onChange={(e) =>
                    updateParticipantRole(p.email, e.target.value as ParticipantRole)
                  }
                  style={{
                    fontSize: 11,
                    padding: '2px 6px',
...
export default function UploadPage() {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      const { data } = await api.post<UploadedDoc>('/documents/upload', form, {
    ...
    <div className="gs-page-narrow">
      ...
      <div className="gs-stepper">
        <div className={`gs-step${step === 1 ? ' is-active' : ' is-done'}`}>
          <div className="gs-step-num">{step > 1 ? <CheckIcon /> : '1'}</div>
