...
import { AddParticipantBody } from '@greedylabs/greedysign-shared';
...
type SigningStatus = 'not_started' | 'in_progress' | 'completed' | 'declined' | string;
...
// ─── 상수 ─────────────────────────────────────────────────
const ROLE_LABEL: Record<string, string> = { signer: '서명자', cc: '참조' };
const ROLE_BADGE: Record<string, string> = { signer: 'badge-primary', cc: 'badge-neutral' };
...
const SIGN_BADGE: Record<string, { label: string; cls: string }> = {
...
// ─── ShareModal ────────────────────────────────────────────
export default function ShareModal({ docId, docStatus, onClose }: ShareModalProps) {
  const isDraft = docStatus === 'draft';

  const [participants, setParticipants] = useState<Participant[]>([]);
  const [inputValue, setInputValue] = useState('');
  const [selectedRole, setSelectedRole] = useState<ParticipantRole>('signer');
  ...
  const loadParticipants = useCallback(async () => {
    try {
      const { data } = await api.get<Participant[]>(`/documents/${docId}/participants`);
  ...
  const handleInputChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    searchTimer.current = setTimeout(async () => {
      try {
        const { data } = await api.get<UserSuggestion[]>(
          `/auth/users/search?q=${encodeURIComponent(val.trim())}`,
        );
        setSuggestions(
          data.filter(
            (u) => !pendingEmails.includes(u.email) && !existingEmails.has(u.email),
          ),
        );
      } catch {
        /* silent */
  ...
  const removePending = (addr: string) =>
    setPendingEmails((prev) => prev.filter((e) => e !== addr));
  ...
  const handleAdd = async (e: FormEvent<HTMLFormElement>) => {
    ...
    for (const addr of emails) {
      ...
      if (!parsed.success) {
        ...
        errors.push(`${addr}: ${first?.message ?? '유효하지 않은 입력'}`);
        continue;
      }
      try {
        await api.post(`/documents/${docId}/participants`, parsed.data);
      } catch (err) {
        const msg =
          err instanceof ApiError
            ? ((err.response.data as { error?: string } | undefined)?.error ?? '추가 실패')
            : '추가 실패';
  ...
  const handleRemove = async (participant: Participant) => {
    ...
    try {
      ...
      const msg =
        err instanceof ApiError
          ? ((err.response.data as { error?: string } | undefined)?.error ?? '제거 실패')
          : '제거 실패';
      alert(msg);
    }
  };
  ...
  // ─── 역할 변경 ─────────────────────────────────────────
  const handleRoleChange = async (participant: Participant, newRole: string) => {
    try {
      const { data } = await api.patch<{ role: ParticipantRole }>(
        `/documents/${docId}/participants/${participant.id}`,
        { role: newRole },
      );
            ...
            {participants.length === 0 ? (
              ...
              <div className="col gap-2">
                {participants.map((p, idx) => {
                  ...
                  const ib = INVITE_BADGE[p.invite_status ?? 'pending'] ?? INVITE_BADGE.pending;
                  const sb =
                    SIGN_BADGE[p.signing_status ?? 'not_started'] ?? SIGN_BADGE.not_started;
